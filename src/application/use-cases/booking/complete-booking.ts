...
import type {
  ...
  CompleteBookingOutput,
} from "./complete-booking.types";
...
export class CompleteBookingUseCase {
  ...
  async execute(input: CompleteBookingInput): Promise<CompleteBookingOutput> {
    try {
  ...
  private async authorize(
    ...
    if (user.role !== "OWNER" && user.role !== "STAFF") {
      return failure(
        new UnauthorizedBookingActionError("Only tenant staff can mark bookings as completed."),
      );
    }
    return success(true);
  }
